---
title: "Cardiovascular model in Appendix 5.2 | Simulation study"
output:
  html_document:
    df_print: paged
...
First, we load the posterior samples from the fitted models obtained by the models in 'WK_simulation_study.Rmd'.
...
### Model 1 (no-without delta $\delta(t)$, Figure 7)
...
### Model 2 (no-with $\delta(t)$, Figure 7)
...
### Model 3 (yes/common $\delta(t)$, Figure 7)
...
```{r}
# P_nwd, P_ysd, P_ycd, P_nnd
rmse = function(actual,pred) sqrt(mean((actual - pred)^2))
pred_nnd = P_nnd[,c("mean", "lower", "upper")]
pred_nwd = P_nwd[,c("mean", "lower", "upper")]
pred_ycd = P_ycd[,c("mean", "lower", "upper")]
pred_ysd = P_ysd[,c("mean", "lower", "upper")]
N_pred=nP_pred
...
ind_pred = round(seq(1,length(time), length.out = nP_pred))

rmse_nnd = rmse_nwd = rmse_ycd = rmse_ysd = rep(NA,Ns)
for(i in 1:Ns){
 rmse_nnd[i] = rmse(df_true[ind_pred,i], pred_nnd[pred_nnd$ID == i,"mean"])
 rmse_nwd[i] = rmse(df_true[ind_pred,i], pred_nwd[pred_nwd$ID == i,"mean"])
 rmse_ycd[i] = rmse(df_true[ind_pred,i], pred_ycd[pred_ycd$ID == i,"mean"])
 rmse_ysd[i] = rmse(df_true[ind_pred,i], pred_ysd[pred_ysd$ID == i,"mean"])
}

rmses_WK2_sim = colMeans(cbind(rmse_nnd,rmse_nwd,rmse_ycd,rmse_ysd))
df_rmses_WK2_sim=data.frame(matrix(rmses_WK2_sim,nrow = 1))
colnames(df_rmses_WK2_sim) = c("nnd", "nwd", "ycd", "ysd")
rownames(df_rmses_WK2_sim) = "WK simulation"
round(df_rmses_WK2_sim,2)
# saveRDS(df_rmses_WK2_sim, "Data/rmse_WK2_sim.rds")
```
...
lbl_post=c(TeX('no-without $\\delta(t)$'),TeX('no-with $\\delta(t)$')
           ,TeX('yes/common $\\delta(t)$'),TeX('yes/shared $\\delta(t)$'))
