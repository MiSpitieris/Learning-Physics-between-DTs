...
parameters {
  ...
  real<lower=0,upper=10> sigmaP[Ns];
  real<lower=0,upper=40> sigmaI[Ns];
...
model {
  ...
  vector[nP + nI] mu[Ns];
  for (s in 1:Ns) {
    L_K[s] = K_wk2(tP[s], tI[s], rho[s], alpha[s], p[s], sigmaP[s], sigmaI[s], rho_d[s], alpha_d[s], R[s], C[s], to_vector(yI[s]));
