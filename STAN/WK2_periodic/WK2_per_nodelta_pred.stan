...
generated quantities {
  ...
  for(n in 1:N_samples) {
    f_P = Pgp_pred_rng(tP, tI, yP, yI, tP_pred[1:nP_pred], rho[n], alpha[n], p, R[n], C[n], sigmaP[n], sigmaI[n]);
    for(np in 1:nP_pred) {
  ...
  for(n in 1:N_samples) {
    f_I = Igp_pred_rng(tP, tI, yP, yI, tP_pred[1:nI_pred], rho[n], alpha[n], p, R[n], C[n], sigmaP[n], sigmaI[n]);
    for(np in 1:nI_pred) {
